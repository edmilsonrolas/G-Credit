 ...
 */
package Views.Painels.Componentes;

import Views.Utilitarios.Util;
...
public class PasswordStrengthStatus extends JPanel {
    ...
    private void checkPassword(String password){
        this.type = password.isEmpty()?0: Util.verifPassword(password);
        if(type == 0){
            label.setText("none");
            label.setVisible(false);
        }else{
            ...
            if(type==1){
                label.setText("Muito fraca");
            }else if(type==2){
                label.setText("Razoavel");
            }else{
                label.setText("Segura");
            }
    ...
    public void initPasswordField(JPasswordField txt){
        ...
        if(pf!=null){
            pf.getDocument().removeDocumentListener(dl);
        }
